...
export function AdminDashboard({ activeTab }: AdminDashboardProps) {
  const {
    reads: { useDisputeWindow, useAuctionContract, useMarketplace, useOwner: useEscrowOwner, useEscrowCounter, useEscrowById, useEscrowRaw, useBuyerEscrows, useSellerEscrows, useDisputeTimeRemaining, useIsDisputeWindowOpen },
  ...
  const {
    reads: { useProductCount, useOrderCount, useProducts, useProduct, useOrder, useBuyerOrders, useSellerOrders, useSellerProducts, useIsSeller, useSeller, useEscrowContract, useOwner: useMarketplaceOwner, useEscrowToOrder, useAllSellers },
  ...
  const allSellers = useAllSellers();
  const eachSeller = allSellers.data?.map((seller) => useSeller(seller)) || [];

  console.log('any seller ?', eachSeller)
  // const seller = useSeller(eachSeller)
        ...
        {escrow.data && escrow.data[0] === 0n ? (
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><strong>Status:</strong> {escrow?.data?.[6] !== undefined ? EscrowStatus[escrow?.data?.[6]] : 'Unknown Status'}</div>
              <div><strong>Dispute Raised:</strong> {escrow?.data?.[8] ? 'Yes' : 'No'}</div>
              <div><strong>Created:</strong> {escrow?.data?.[7] ? new Date(Number(escrow?.data?.[7]) * 1000).toLocaleString() : 'N/A'}</div>
              <div><strong>Resolver:</strong> {escrow?.data?.[9] || 'N/A'}</div>
            </div>
          </div>
        )}
      </div>
